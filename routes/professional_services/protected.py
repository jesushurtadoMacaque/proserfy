...
from schemas.profesional_service_schema import ImageUpdatedResponse, ProfessionalServiceCreate, ProfessionalServiceResponse
...
async def create_professional_service(service: ProfessionalServiceCreate, db: db_dependency, current_user: User = Depends(get_current_active_user)):
    ...
    db_service = ProfessionalService(**service.model_dump(), professional_id=current_user.id)
    db.add(db_service)
    db.commit()
...
async def upload_images(
    ...
    if not professional_service or professional_service.professional_id != current_user.id:
...
async def delete_image(image_id: int, db: db_dependency, current_user: User = Depends(get_current_active_user)):
