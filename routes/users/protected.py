...
from schemas.user_schema import ChangePasswordRequest, CompleteProfile, UserResponse, ChangeRoleRequest, SuspendUserRequest, UserUpdate
...
async def get_current_active_user(request: Request, db: db_dependency, current_user: str = Depends(get_current_user)):
    user = get_user_by_email(db, current_user)
    if not user:
        raise GenericException(message="User not exists", code=status.HTTP_404_NOT_FOUND)
    if not user.is_active:
        raise GenericException(message="User is suspended", code=status.HTTP_403_FORBIDDEN)
    return user

@router.put("/users", tags=["users"], response_model=UserResponse, responses=validation_error_response)
async def update_user(user_update: UserUpdate, db: db_dependency, current_user: User = Depends(get_current_active_user)):
...
async def change_user_role(db: db_dependency, request: ChangeRoleRequest, current_user: User = Depends(get_current_active_user)):
...
async def suspend_user(db: db_dependency, request: SuspendUserRequest, current_user: User = Depends(get_current_active_user)):
...
async def change_password(change_password_request:ChangePasswordRequest, db:db_dependency, current_user:User = Depends(get_current_active_user)):

    if not current_user.password: 
        raise GenericException(message="You are registered by social", code=status.HTTP_401_UNAUTHORIZED)
    
    if not verify_password(change_password_request.current_password, current_user.password):
...
async def complete_profile(complete_profile:CompleteProfile, db: db_dependency, current_user:User = Depends(get_current_active_user)):
